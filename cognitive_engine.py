...
def find_prerequisite(topic, db_driver):
    query = "MATCH (m:MicroSkill {name: $topic})-[:REQUIRES_PREREQUISITE]->(p:MicroSkill) RETURN p.name AS prereq LIMIT 1"
...
def generate_draft_script(student_name, grade, topic, prereq, interest, ai_client):
    ...
    prompt = f"You are Ananya, an AI tutor. Student: {student_name}, Grade: {grade}. Topic: '{topic}'. Interest: '{interest}'. {scaffold} ONLY teach Grade {grade} concepts. Write a 3-sentence script using an analogy about '{interest}'. NO MARKDOWN."
...
def shadow_student_veto(script, grade, interest, ai_client):
    prompt = f"You are a distracted {grade}yr old obsessed with {interest}. Read this: '{script}'. Is it boring? Return ONLY JSON: {{"approved": true/false, "feedback": "reason"}}"
    res = ai_client.chat.completions.create(model="gpt-4o", messages=[{"role": "user", "content": prompt}], response_format={"type":"json_object"})
...
def generate_audio_base64(script):
    ...
    return base64.b64encode(req.content).decode() if req.status_code == 200 else None
